describe Chess::Board do
  describe '#move' do
    context 'supports algebraic moves' do
      let(:positions) do
        [
      ...
      it 'returns the correct move count' do
        positions.each do |position|
          chess = Chess.new(fen: position[:fen])

          position[:moves].each do |move|
            chess.move(move)
          end

          expect(chess.moves.count).to eq(position[:moves].count)
        end
      end
    end
  end
    ...
    context 'valid fen' do
      ...
      it 'loads the fen' do
        positions.each do |position|
          ...
          expect(chess.fen).to eq(position[:fen])
        end
      end
    end

    context 'invalid fen' do
      let(:positions) do
        [
      ...
      it 'raises errors on bad fens' do
        positions.each do |position|
          expect(Chess::Board.new(fen: position[:fen])).to raise_error
        end
      end
    end
  end

  describe '#load_pgn' do
    context 'good pgn' do
      let(:games) do
        [
          {pgn: [
      ...
      it 'returns true' do
        games.each do |game|
          chess = Chess.newxw
          expect(chess.load_pgn(game[:pgn])).to be_true
        end
      end
    end
    ...
    context 'bad pgn' do
      ...
      it 'returns false' do
        games.each do |game|
          ...
          expect(chess.load_pgn(game[:pgn])).to be_false
        end
      end
    end
  end

  describe '#valid_fen?' do
    let(:positions) do
      [
    ...
    it 'returns true if valid and returns false if invalid' do
      positions.each do |position|
        if position[:error_number] == 0
          expect(chess.valid_fen?(position[:fen])).to eq(true)
        else
          expect(chess.valid_fen?(position[:fen])).to eq(false)
        end
      end
    end
  end
